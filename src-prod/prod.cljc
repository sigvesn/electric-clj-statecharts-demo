...
#?(:clj
   (defn -main [& {:strs [] :as args}] ; clojure.main entrypoint, args are strings
     ...
     (jetty/start-server!
       (fn [ring-req] (e/boot-server {} app.statechart-clj-demo/Main ring-req))
       config)))
...
#?(:cljs
   (do
     (def electric-entrypoint (e/boot-client {} app.statechart-clj-demo/Main nil))
     (defn ^:export start! []
       (electric-entrypoint
